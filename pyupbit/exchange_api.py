...
class Upbit:
    ...
    def sell_limit_order(self, ticker, price, volume, contain_req=False):
        ...
        try:
            ...
            print(x.__class__.__name__)
            return None
    def cancel_and_new(self,prev_order_uuid,new_price,new_volume,new_ord_type="limit",contain_req=False):
        try:
    ...
    def cancel_all(self,cancel_side="all",pairs=None):
        try:
            ...
            result = requests.delete(f"{BASE_URL}{PATH}", headers=headers, params=params)
            return result.json()
...
if __name__ == "__main__":
    ...
    UPBIT_ACCESS_KEY = "4JNi2AeoViUgUfabaxWJTVEXOpLsMsF7VgIhx8bo"
    UPBIT_SECRET_KEY = "pQmMZCWjLlgAr2C0rru9h3KR5cHSqXCkaWCuxrbW"
    upbit = Upbit(UPBIT_ACCESS_KEY, UPBIT_SECRET_KEY)
    #print(upbit.get_balances())
    resp = upbit.cancel_all(pairs="USDT-XRP")
    print("취소:",resp)
    time.sleep(1)
    # order 
    resp = upbit.buy_limit_order("USDT-XRP", 1, 5)
    print(resp['uuid'])
    time.sleep(1)
    resp = upbit.cancel_and_new(resp['uuid'], 1.2, 5)
    print("취소후 재매수:",resp)
    #-------------------------------------------------------------------------
    # 자산 
    #     전체 계좌 조회 
